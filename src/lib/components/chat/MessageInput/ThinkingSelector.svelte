<script lang="ts">
	...
	$: tooltipText = modelLabel ? `${$i18n.t('Thinking')} · ${modelLabel}` : $i18n.t('Thinking');
			...
			<div slot="content">
				<DropdownMenu className="min-w-44">
					<div class="px-2.5 pt-1.5 pb-1">
						<div class="text-[0.625rem] font-normal uppercase tracking-wider text-gray-400 dark:text-gray-500">
							{$i18n.t('Thinking')}
						</div>
						{#if modelLabel}
							<div class="mt-0.5 truncate text-[0.6875rem] font-normal text-gray-500 dark:text-gray-400">
								{modelLabel}
							</div>
						{/if}
					</div>
